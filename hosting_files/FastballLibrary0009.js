...
function fbiLibGetDomainName(url) {
	if (fbiLibDomainName && !url) {
		return;
	}
	else {
		var a = url || window.location.hostname;
		...
		var environment = a.match(/test\.|dev\.|debug\./i) || [""];
		fbiLibLanguage = a.match(/^s?[-\.]?[A-z]{2}\./i) || [""];
		fbiLibDomainName = a.lastIndexOf('.') === a.indexOf('.') ? a : environment[0] + a.substr(a.lastIndexOf('.', a.lastIndexOf('.') - 1) + 1);
	}
}
...
function fbiLibSetDomainName(a) {
	fbiLibGetDomainName(a);
}

function fbiLibInsertIntoQueue(a) {
	fbiLibGetDomainName();
	var baseURL = window.location.protocol + "//" + fbiLibLanguage[0] + "img.";
	fbiLibQueue.push(baseURL + fbiLibDomainName + "/pageevents.aspx?" + a);
}

function fbiGetPathnameFromImg() {
	var imgsrc;
	for (i = 0; i < document.images.length; i++) {
		imgsrc = document.images[i].src.toLowerCase();
		if (imgsrc.indexOf("image.aspx") != -1) {
			var page = imgsrc.split("?")[1].split("&");
			for (p = 0; p < page.length; p++) {
				if (page[p].substring(0, 5) == "page=") {
					return decodeURIComponent(page[p].substring(5));
				}
			}
		}
	}
...
function fbiRecordFastballOfferClick(offerid, redirectUrlOrOnCompleteFunction) {
	...
	if (redirectUrlOrOnCompleteFunction !== null && typeof redirectUrlOrOnCompleteFunction !== 'undefined') {
...
function fbiRecordFastballOfferEvent(offevt /*i=impression, c=click*/, offerid, onCompleteFunction) {
	fbiLibGetDomainName();
	var baseURL = window.location.protocol + "//" + fbiLibLanguage[0] + "img.";
	var fbiRecordFastballOfferEventUrl = baseURL + fbiLibDomainName + "/offerevent.aspx?offevt=" + offevt +
